...
export class AdminService {
    ...
    async updateCompanyStatus(id, status) {
        const result = await executeQuery('UPDATE companies SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [status, id]);
        return result.rows[0];
    }
    async adjustCompanyCredits(id, amount) {
        const result = await executeQuery('UPDATE companies SET credits = credits + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [amount, id]);
        return result.rows[0];
    }
    async updateUserStatus(id, isActive) {
        const result = await executeQuery('UPDATE users SET is_active = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [isActive, id]);
        return result.rows[0];
    }
    async updateUserRole(id, role) {
        const result = await executeQuery('UPDATE users SET role = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [role, id]);
        return result.rows[0];
    }
