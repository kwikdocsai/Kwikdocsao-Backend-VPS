...
async function fixAllFileUrls() {
    ...
    const result = await executeQuery(`SELECT id, file_name, file_url, storage_path, bucket_name 
         FROM documents 
    ...
    for (const doc of result.rows) {
        ...
        if (doc.file_url !== correctUrl) {
            ...
            await executeQuery(`UPDATE documents SET file_url = $1 WHERE id = $2`, [correctUrl, doc.id]);
