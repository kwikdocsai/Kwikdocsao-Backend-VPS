...
async function testQueries() {
    try {
        ...
        const docs1 = await pool.query('SELECT id, file_name, company_id, tipo_movimento, valor_documento FROM documents WHERE company_id = $1', [user.company_id]);
        ...
        const docs2 = await pool.query('SELECT id, file_name, company_id, tipo_movimento, valor_documento FROM documents WHERE company_id = (SELECT company_id FROM users WHERE id = $1)', [user.id]);
        console.log(`Found ${docs2.rowCount} documents using subquery`);
        // Test 3: Check view_fiscal_analytics
        console.log('\n--- Test 3: Check view_fiscal_analytics ---');
        try {
            const analytics = await pool.query('SELECT * FROM view_fiscal_analytics WHERE company_id = $1 ORDER BY mes_referencia DESC LIMIT 3', [user.company_id]);
        ...
        const financialRes = await pool.query(`SELECT 
                SUM(CASE 
