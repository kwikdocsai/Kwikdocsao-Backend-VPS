...
app.post('/api/documents/analyze-proxy', requireAuth, upload.array('files'), async (req, res) => {
    try {
        ...
        const savedDocs = await Promise.all(files.map(async (file, idx) => {
            ...
            return { id: docId, fileName: file.originalname, fileUrl: uploadResult.url };
        }));
        // 2. Resolve Webhook
...
app.post('/api/admin/server-plans', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { slug, name, description, monthly_credits, price, analysis_cost, max_users, color, features, is_visible, welcome_credits, analysis_time, cpu_cores, ram_gb, gpu_info } = req.body;
        const id = crypto.randomUUID();
        const result = await executeQuery(`INSERT INTO server_performance_plans 
...
app.patch('/api/admin/server-plans/:id', requireAuth, requireAdmin, async (req, res) => {
    try {
        const fields = [];
        const values = [];
        const body = req.body;
        ...
        const result = await executeQuery(`UPDATE server_performance_plans SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${idx} RETURNING *`, values);
        if (result.rowCount === 0)
...
app.delete('/api/admin/server-plans/:id', requireAuth, requireAdmin, async (req, res) => {
    try {
...
app.post('/api/workflow/trigger', authenticate, upload.array('files'), async (req, res) => {
    try {
        console.log(`[/api/workflow/trigger] Received ${req.files?.length || 0} files`);
        ...
        setImmediate(async () => {
            try {
                ...
                await workflowService.triggerAnalysis(req.files, req.body.metadata, req.user);
