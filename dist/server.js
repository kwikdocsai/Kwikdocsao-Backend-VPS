...
app.post('/api/webhooks/n8n/completed', async (req, res) => {
    try {
        const body = req.body;
        // Resilient ID extraction
        const docId = body.docId || body.doc_id || body.audit_id || body.id || (body.metadata && (body.metadata.docId || body.metadata.audit_id));
        const userId = body.userId || body.user_id || (body.metadata && body.metadata.user_id);
        const companyId = body.companyId || body.company_id || (body.metadata && body.metadata.company_id);
        console.log(`[WEBHOOK] Received callback. ID: ${docId}, User: ${userId}`);
        if (!docId) {
            return res.status(400).json({ error: 'No document ID found in webhook body' });
        }
        // Result Logic
        const docCheck = await executeQuery('SELECT id FROM documents WHERE id = $1', [docId]);
        if (docCheck.rowCount && docCheck.rowCount > 0) {
            console.log(`[WEBHOOK] Updating documents table for ${docId}`);
            const result = await documentsService.completeAnalysis(docId, body, userId);
            return res.json({ success: true, type: 'document', doc: result });
        }
        // faturix_audits table deleted
        return res.json({ success: true, message: 'Processamento concluído (tabela faturix_audits removida)' });
        // Fallback or error
        console.warn(`[WEBHOOK] ID ${docId} not found in any table.`);
        res.status(404).json({ error: 'Document or Audit not found' });
    }
    catch (err) {
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // Log Login (Non-blocking to avoid crash if logging fails)
...
app.post('/api/documents/analyze-proxy', requireAuth, upload.array('files'), async (req, res) => {
    try {
        ...
        // Fetch company name for folder structure
        const companyRes = await executeQuery('SELECT name FROM companies WHERE id = $1', [targetCompanyId]);
        ...
        // 4. Forward to N8N (FIRE AND FORGET vs AWAIT)
        // If we want persistence across refresh, we should probably return immediately
        // and let n8n call us back via Webhook. 
        // But for compatibility with existing frontend expectations, we'll await the response for now
        // BUT the documents are already in the DB, so refresh will work anyway.
        const n8nResponse = await fetch(webhookUrl, {
            method: 'POST',
            body: formData
        });
        if (!n8nResponse.ok) {
            ...
            console.error(`[Proxy Analyze] N8N Error (${n8nResponse.status}): ${errText}`);
            // Don't fail the whole request if we already saved the records, 
            // but return error so frontend knows.
            return res.status(n8nResponse.status).send(errText);
        }
        const data = await n8nResponse.json();
        // Include the saved database records (with S3 URLs) in the response
        // so the frontend can update its local state immediately.
        res.json({
            n8nResponse: data,
            documents: savedDocs
        });
...
app.use((err, req, res, next) => {
    ...
    res.status(status).json({
        error: (isInternal && !isDev) ? 'Ocorreu um erro interno de processamento.' : err.message,
        message: isDev ? err.message : (isInternal ? 'Consulte o log do sistema para detalhes.' : err.message),
