...
import { saftService } from './services/saft.service.js';
...
// ... (imports)
// Helper
const logSystemAction = async (userId, action, resource, target_id, details, req, companyId) => {
    try {
        await executeQuery(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details, ip_address, user_agent, company_id) 
...
async function createAiAlert(companyId, agent, severity, title, message, metadata = {}) {
    try {
        await executeQuery(`INSERT INTO ai_alerts (company_id, agent_name, severity, title, message, metadata) 
             VALUES ($1, $2, $3, $4, $5, $6)`, [companyId, agent, severity, title, message, JSON.stringify(metadata)]);
    }
    catch (err) {
...
// DIAGNOSTIC: Verify if SQL fix is deployed
app.get('/api/diagnostic/sql-version', (req, res) => {
    // This endpoint helps verify if the SQL ambiguity fix is deployed
    // If this returns "FIXED", the server has the corrected query
    const querySignature = 'SELECT c.name, c.credits, u.role FROM companies c JOIN users u';
    res.json({
        status: 'FIXED',
        signature: querySignature,
        timestamp: new Date().toISOString(),
        message: 'SQL ambiguity fix is deployed'
    });
});
// DIAGNOSTIC: Debug Document Status
app.get('/api/diagnostic/debug-doc/:filename', async (req, res) => {
    try {
        const filename = req.params.filename;
        const result = await executeQuery('SELECT id, file_name, status, created_at FROM documents WHERE file_name ILIKE $1 ORDER BY created_at DESC LIMIT 10', [`%${filename}%`]);
...
app.use('/api/admin/server-plans', serverPlansRoutes); // [FIX] Redirect admin legacy calls to Router
app.get('/api/servers/status', requireAuth, async (req, res) => {
    try {
        ...
        const targetCompanyId = queryCompanyId || req.user.companyId;
...
app.post('/api/servers/activate', requireAuth, async (req, res) => {
    try {
        ...
        if (!hasAccess && (role === 'ACCOUNTANT' || isOwner)) {
            ...
            const checkRes = await executeQuery('SELECT id FROM companies WHERE id = $1 AND owner_id = $2', [targetCompanyId, userId]);
...
// Unified N8N Callback Webhook (Supports Arrays, Batches, JSON & Multipart)
// Unified N8N Callback Webhook (Async)
app.post('/api/webhooks/n8n/completed', upload.any(), async (req, res) => {
    try {
        let body = req.body;
        console.log(`[WEBHOOK] Received payload. Keys: ${Object.keys(body).join(', ')}`);
        // FIELD PARSING FIX: Unpack 'resultado_analise' (String or Object)
        // This is critical because N8N might send it as a stringified JSON
        if (body.resultado_analise) {
            try {
                if (typeof body.resultado_analise === 'string') {
                    const parsed = JSON.parse(body.resultado_analise);
                    body = { ...body, ...parsed };
                }
                else if (typeof body.resultado_analise === 'object') {
                    body = { ...body, ...body.resultado_analise };
                }
                console.log('[WEBHOOK] Successfully unpacked analysis data.');
            }
            catch (e) {
                console.error('[WEBHOOK] Failed to unpack analysis data:', e);
            }
        }
        // Support Legacy Array Format (Resilient)
        const items = Array.isArray(body) ? body : [body];
        for (const item of items) {
            // Resilient ID extraction
            const docId = item.docId || item.doc_id || item.audit_id || item.id || (item.metadata && (item.metadata.docId || item.metadata.audit_id));
            const userId = item.userId || item.user_id || (item.metadata && item.metadata.user_id);
            if (!docId) {
                console.warn('[WEBHOOK] Skipping item without docId');
                continue;
            }
            console.log(`[WEBHOOK] Processing docId: ${docId}, User: ${userId}`);
            // Result Logic
            // We use 'item' (the parsed object) not 'req.body'
            await documentsService.completeAnalysis(docId, item, userId);
        }
...
// ==================== AGENTS & PROMPTS MANAGEMENT ====================
app.get('/api/admin/agents', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/agents/:id', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
        const { prompt_text, model_override, temperature, is_active } = req.body;
        await executeQuery(`UPDATE agent_prompts 
             SET prompt_text = $1, model_override = $2, temperature = $3, is_active = $4, updated_at = NOW(), updated_by = $5
             WHERE id = $6`, [prompt_text, model_override, temperature, is_active, req.user.id, req.params.id]);
...
app.get('/api/agents/prompt/:slug', requireAuth, async (req, res) => {
    try {
        const result = await executeQuery('SELECT prompt_text, model_override, temperature FROM agent_prompts WHERE agent_slug = $1', [req.params.slug]);
...
app.get('/api/admin/engagement', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/growth', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/usage', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/infra', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/storage-config', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/storage-config', requireAuth, requireSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`INSERT INTO storage_settings (provider, endpoint, region, bucket, access_key, secret_key, tenant_id, is_active)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`, [provider, endpoint, region, bucket, access_key, secret_key, tenant_id, is_active || false]);
...
app.put('/api/admin/storage-config/:id/activate', requireAuth, requireSuperAdmin, async (req, res) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
...
app.patch('/api/admin/companies/:id', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.patch('/api/admin/users/:id/status', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/users/bulk-update', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.get('/api/admin/users/:id/activity', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/users/impersonate/:id', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/payments/:id/approve', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/payments/:id/reject', requireAuth, requireSuperAdmin, async (req, res) => {
    try {
...
app.post('/api/admin/bank-accounts', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { bank_name, account_number, iban, swift, holder_name } = req.body;
        const result = await executeQuery(`INSERT INTO bank_accounts (bank_name, account_number, iban, swift, holder_name, is_active) 
             VALUES ($1, $2, $3, $4, $5, true) 
             RETURNING *`, [bank_name, account_number, iban, swift, holder_name]);
...
app.delete('/api/admin/bank-accounts/:id', requireAuth, requireAdmin, async (req, res) => {
    try {
...
app.post('/api/setup/company', requireAuth, async (req, res) => {
    try {
        ...
        // 1. Create Company
        const companyRes = await executeQuery(`INSERT INTO companies (name, nif, company_type, tax_regime, main_activity, owner_id, status, credits) 
             VALUES ($1, $2, $3, $4, $5, $6, 'VALIDATE', 0) 
             RETURNING *`, [name, nif, type || 'PME', regime || 'Geral', activity, req.user.id]);
        const newCompany = companyRes.rows[0];
        // 2. Link User to Company
        await executeQuery(`UPDATE users SET company_id = $1 WHERE id = $2`, [newCompany.id, req.user.id]);
        // 3. Create Main Fiscal Entity (Self)
        await executeQuery(`INSERT INTO fiscal_entities (company_id, name, nif, entity_type, status) 
             VALUES ($1, $2, $3, 'CLIENT', 'ACTIVE')`, [newCompany.id, name, nif]);
        // 4. Log Action
...
app.post('/api/system/audit-batch', requireAuth, async (req, res) => {
    try {
        ...
        await logSystemAction(req.user.id, 'FISCAL_VALIDATION_BATCH', 'compliance', company_id, details, req, company_id);
...
app.get('/api/system/compliance-logs', requireAuth, async (req, res) => {
    try {
...
app.post('/api/team/member', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const planLimitRes = await executeQuery(`SELECT spp.max_users 
             ...
             ORDER BY ss.created_at DESC LIMIT 1`, [targetCompanyId]);
        ...
        const result = await executeTransaction(async (client) => {
            ...
            const insertRes = await client.query(`INSERT INTO users (name, email, password, role, company_id, owner_id, permissions, status, credits, plan, is_active) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7, 'ACTIVE', 0, $8, true) 
                 RETURNING id, name, email, role, status, created_at, credits`, [name, email, hashedPassword, validRole || 'COLLABORATOR', targetCompanyId, req.user.id, JSON.stringify(permissions || {}), licensePlan || 'free']);
            const newUser = insertRes.rows[0];
            console.log('[DEBUG] TX: User Inserted:', newUser?.id);
            // 6. Log Transaction
            console.log('[DEBUG] TX: Logging Transaction...');
            await client.query(`INSERT INTO transactions (company_id, user_id, credits, type, status, amount, description, metadata, plan_name) 
...
app.put('/api/team/permissions/:id', requireAuth, async (req, res) => {
    try {
...
app.post('/api/users/:id/credits', requireAuth, async (req, res) => {
    try {
        ...
        const result = await executeQuery('UPDATE users SET credits = credits + $1 WHERE id = $2 RETURNING credits, name', [amount, userId]);
        if (result.rowCount === 0)
            return res.status(404).json({ error: 'User not found' });
        // Log transaction
        await executeQuery(`INSERT INTO transactions (company_id, user_id, credits, type, description, amount) 
              VALUES ($1, $2, $3, 'BONUS', $4, 0)`, [req.user.companyId || req.user.id, userId, amount, `Recarga Manual por ${req.user.name}`]);
...
app.post('/api/credits/transfer-to-user', requireAuth, async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        await client.query(`INSERT INTO transactions (company_id, user_id, credits, type, description, amount) 
             VALUES ($1, $2, $3, 'TRANSFER_OUT', $4, 0)`, [sourceCompanyId, req.user.id, amount, `Transferência para usuário ${targetUserId}`]);
        ...
        await client.query(`INSERT INTO transactions (company_id, user_id, credits, type, description, amount, status) 
             VALUES (NULL, $1, $2, 'TRANSFER_IN', $3, 0, 'COMPLETED')`, [targetUserId, amount, `Recebido de ${companyName}`]);
...
// Update User Prompts
app.put('/api/users/prompts', requireAuth, async (req, res) => {
    try {
        const { prompts } = req.body;
        if (!prompts)
            return res.status(400).json({ error: 'Prompts required' });
        // Update separate columns
        await executeQuery(`UPDATE users SET 
                auditor_prompt = $1,
                strategist_prompt = $2,
                rag_prompt = $3,
                analyzer_prompt = $4,
...
// ==================== FATURIX API ====================
app.get('/api/faturix/audits', requireAuth, async (req, res) => {
    try {
...
app.get('/api/faturix/stats', requireAuth, async (req, res) => {
    try {
        const { companyId, month, year } = req.query;
        // Tenancy Check
        if (!(await validateCompanyAccess(req.user, companyId))) {
            return res.status(403).json({ error: 'Acesso negado a esta empresa' });
        }
        const stats = await authService.getFaturixStats(req.user.id, companyId, month ? parseInt(month) : undefined, year ? parseInt(year) : undefined);
        res.json(stats);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/faturix/audits', requireAuth, async (req, res) => {
    try {
...
app.patch('/api/faturix/audits/:id/status', requireAuth, async (req, res) => {
    try {
...
app.post('/api/faturix/analyze', requireAuth, upload.array('files'), async (req, res) => {
    try {
        ...
        for (const { id, file } of auditIds) {
            ...
            const blob = new Blob([new Uint8Array(content)], { type: file.mimetype });
        ...
        if (!n8nResponse.ok) {
            ...
            const usedResultsIndices = new Set();
            for (let fileIdx = 0; fileIdx < auditIds.length; fileIdx++) {
                ...
                if (resultIndex === -1) {
                    resultIndex = resultsArray.findIndex((r, rIdx) => {
                        if (usedResultsIndices.has(rIdx))
                            return false;
        ...
        try {
            ...
            // Cleanup Logic
            files.forEach(f => fs.unlink(f.path, (err) => { if (err)
...
app.post('/api/faturix/callback', async (req, res) => {
    try {
        const { audit_id, analysis_result } = req.body;
        if (!audit_id || !analysis_result)
...
app.get('/api/faturix/rules', requireAuth, async (req, res) => {
    try {
...
app.post('/api/faturix/rules', requireAuth, async (req, res) => {
    try {
...
app.patch('/api/faturix/rules/:id/toggle', requireAuth, async (req, res) => {
    try {
        const { isActive } = req.body;
...
app.get('/api/faturix/settings', requireAuth, async (req, res) => {
    try {
...
app.get('/api/fiscal/entities', requireAuth, async (req, res) => {
    try {
        ...
        const entities = await authService.getFiscalEntities(req.user.id, companyId);
        res.json(entities);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/fiscal/saft', requireAuth, async (req, res) => {
    try {
        const { companyId, month, year } = req.query;
        if (!companyId || !month || !year) {
            return res.status(400).json({ error: 'CompanyID, month, and year are required' });
        }
        const result = await authService.generateSaft(companyId, parseInt(month), parseInt(year));
...
app.post('/api/faturix/settings', requireAuth, async (req, res) => {
    try {
...
app.post('/api/credits/transfer-to-company', requireAuth, async (req, res) => {
    try {
...
app.get('/api/credits/packages', requireAuth, async (req, res) => {
    try {
...
app.get('/api/companies/:id/doc-count', requireAuth, async (req, res) => {
    try {
...
app.post('/api/companies', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (creationCost > 0) {
            ...
            await client.query(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details) 
                 VALUES ($1, $2, $3, $4, $5)`, [req.user.id, 'company_creation_fee', 'finance', 'user_wallet', JSON.stringify({ amount: creationCost, companyName: name })]);
        }
        // 4. Create Company
...
app.delete('/api/companies/:id', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (refundAmount > 0) {
            ...
            // Log Refund
            await client.query(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details) 
                 VALUES ($1, $2, $3, $4, $5)`, [req.user.id, 'refund_credits', 'finance', req.params.id, JSON.stringify({ amount: refundAmount, reason: 'company_deletion' })]);
        }
...
app.put('/api/companies/:id', requireAuth, async (req, res) => {
    try {
        const { name, nif, email, plan, regime, address, phone, website, sector, cae, fiscal_rep, accountant_name, accountant_email, logo_url, fiscal_status, is_defaulter, fiscal_residence } = req.body;
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            if (fiscal_status === 'Activo' && currentCompanyStatus !== 'ACTIVE') {
                ...
                await client.query(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details) 
                     VALUES ($1, $2, $3, $4, $5)`, [req.user.id, 'company_validation_fee', 'finance', 'user_wallet', JSON.stringify({ amount: VALIDATION_COST, companyId: company.id })]);
                // Update Company to ACTIVE, Verified, and Add Bonus
                const updateRes = await client.query(`UPDATE companies SET 
                        status = 'ACTIVE', 
                        is_verified = true, 
                        credits = credits + $1 
                      WHERE id = $2 RETURNING *`, [BONUS_CREDITS, company.id]);
                await client.query('COMMIT');
...
app.post('/api/documents/analyze-proxy', requireAuth, upload.array('files'), async (req, res) => {
    try {
        const files = req.files;
        const { companyId, userId, metadata, preview_url } = req.body;
        if (!files || files.length === 0)
            throw new Error('Nenhum arquivo enviado para o proxy');
        ...
        // FIXED: Ambiguous 'name' and 'credits' columns with 'c.' prefix
        const companyRes = await executeQuery('SELECT c.name, c.credits, u.role FROM companies c JOIN users u ON u.company_id = c.id WHERE c.id = $1 AND u.id = $2', [targetCompanyId, targetUserId]);
        ...
        // 1. Parallel S3 Uploads & DB Pre-save
        const savedDocs = await Promise.all(files.map(async (file, idx) => {
            const fileMeta = metaParsed.find((m) => m.file_name === file.originalname) || metaParsed[idx] || {};
            const docId = fileMeta.audit_id || fileMeta.id || crypto.randomUUID();
            console.log(`[Proxy Analyze] Uploading to S3: ${file.originalname} (ID: ${docId})`);
            // [SRE] Memory Fix: Stream from disk
            const fileStream = fs.createReadStream(file.path);
            const category = storageService.getCategoryByType(fileMeta.type || 'UNKNOWN');
            const uploadResult = await storageService.uploadFile(fileStream, file.originalname, file.mimetype, targetCompanyId, companyName, category);
            // 1.2 Save record with URL
            await executeQuery(`INSERT INTO documents (id, company_id, uploaded_by, file_name, status, type, file_url, storage_path, bucket_name)
                 VALUES ($1, $2, $3, $4, 'PROCESSING', 'UNKNOWN', $5, $6, $7)
                 ON CONFLICT (id) DO UPDATE SET 
                    status = 'PROCESSING',
                    file_url = EXCLUDED.file_url,
                    storage_path = EXCLUDED.storage_path,
                    bucket_name = EXCLUDED.bucket_name`, [docId, targetCompanyId, targetUserId, file.originalname, uploadResult.url, uploadResult.path, uploadResult.bucket]);
            return { id: docId, fileName: file.originalname, fileUrl: uploadResult.url, path: file.path };
        }));
        // 2. Resolve Webhook
        ...
        savedDocs.forEach(sd => formData.append('file_url', sd.fileUrl));
        if (preview_url) {
        ...
        // [SRE] Memory Fix: Read from disk for FormData (released after request)
        for (const file of files) {
            const content = fs.readFileSync(file.path);
            const blob = new Blob([new Uint8Array(content)], { type: file.mimetype });
            formData.append('files', blob, file.originalname);
        ...
        fetch(webhookUrl, {
            ...
            // [SRE] Clean up temp files
            files.forEach(f => {
                fs.unlink(f.path, (err) => { if (err)
                    console.warn('Failed to cleanup temp', f.path); });
            });
        });
        ...
        res.status(202).json({
            ...
            documents: savedDocs.map(({ path, ...rest }) => rest) // Exclude internal path
        });
    }
    catch (err) {
...
app.patch('/api/companies/:id/status', requireAuth, async (req, res) => {
    try {
        ...
        const result = await executeQuery('UPDATE companies SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', [status, req.params.id]);
        if (result.rowCount === 0)
            return res.status(404).json({ error: 'Company not found' });
...
app.get('/api/documents', requireAuth, async (req, res) => {
    try {
        ...
        const limitNum = parseInt(limit) || 50; // Default to 50 for safety
        ...
        // FORCE NO-CACHE for Vercel/Production
...
app.post('/api/documents', requireAuth, async (req, res) => {
    try {
        ...
        const result = await executeQuery(`INSERT INTO documents (company_id, uploaded_by, file_name, file_url, status, type, file_base64) 
             VALUES ($1, $2, $3, $4, 'PROCESSING', $5, $6) RETURNING *`, [company_id, req.user.id, file_name, file_url, type || 'UNKNOWN', file_base64]);
        res.status(201).json(result.rows[0]);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
app.post('/api/documents/sync', requireAuth, async (req, res) => {
    try {
        ...
        const { id, file_name, file_url, company_id, type, file_base64, data, raw_data, status, raw_text, raw_n8n_response, full_analysis } = req.body;
        const d = data || {};
        ...
        try {
            await client.query('BEGIN');
            const insertRes = await client.query(`INSERT INTO documents (
            ...
            if (['ERROR', 'FAILED'].includes(status || '')) {
                await createAiAlert(company_id, 'Watchdog', 'CRITICAL', 'Falha no Processamento', `O documento ${file_name} não pôde ser analisado pela IA. Verifique o formato ou a qualidade da imagem.`, { docId: id });
            }
            await client.query('COMMIT');
...
app.patch('/api/documents/:id/resolve', requireAuth, async (req, res) => {
    try {
        ...
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE documents SET 
                    status = $1, 
                    resolution_notes = $2, 
                    responsible_user_id = $3, 
                    completed_at = now() 
                 WHERE id = $4 RETURNING *`, [status || 'COMPLETED', resolution_notes, userId, docId]);
            if (result.rowCount === 0) {
                await client.query('ROLLBACK');
                return res.status(404).json({ error: 'Document not found' });
            }
            ...
            if (relatedIds && Array.isArray(relatedIds) && relatedIds.length > 0) {
                await client.query(`UPDATE documents SET status = 'ARCHIVED' WHERE id = ANY($1)`, [relatedIds]);
            }
            await client.query('COMMIT');
            ...
            if (status === 'APROVADO_USUARIO') {
                await createAiAlert(result.rows[0].company_id, 'Sentinel', 'INFO', 'Documento Validado Manualmente', `O documento ${result.rows[0].file_name} foi validado manualmente por ${req.user.name || 'um usuário'}.`, { docId, userId: req.user.id });
            }
            res.json(result.rows[0]);
        }
        catch (e) {
...
app.get('/api/team', requireAuth, async (req, res) => {
    try {
        ...
        if (!targetCompanyId) {
            const result = await executeQuery(`SELECT id, name, email, role, plan, credits, status,
        ...
        const result = await executeQuery(`SELECT id, name, email, role, plan, credits, status,
            ...
            ORDER BY created_at DESC`, [targetCompanyId]);
        res.json(result.rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/team/stats', requireAuth, async (req, res) => {
    try {
        ...
        if (!targetCompanyId)
            return res.json({ total_members: 0, active_members: 0, limit: 10 });
        const stats = await executeQuery(`SELECT 
                COUNT(*) as total_members,
                COUNT(*) FILTER (WHERE status = 'ACTIVE') as active_members
             FROM users WHERE company_id = $1 AND role NOT IN ('ADMIN', 'ACCOUNTANT')`, [targetCompanyId]);
        res.json({
...
app.put('/api/team/status/:id', requireAuth, async (req, res) => {
    try {
        const { isActive } = req.body;
        const targetUserId = req.params.id;
        const userId = req.user.id;
        // Verify permission: Must be owner or admin
        const check = await executeQuery(`SELECT u1.company_id as target_comp, u2.company_id as requester_comp, u2.role as requester_role 
             FROM users u1, users u2 
             WHERE u1.id = $1 AND u2.id = $2`, [targetUserId, userId]);
        if (check.rowCount === 0)
            return res.status(404).json({ error: 'User not found' });
...
app.delete('/api/team/member/:id', requireAuth, async (req, res) => {
    try {
        const targetUserId = req.params.id;
        const userId = req.user.id;
        // Verify permission
    ...
    try {
        ...
        statusRes.rows.forEach(row => {
            ...
            if (s === 'APROVADO_USUARIO')
                approvedManual += c;
            else if (s === 'APROVADO' || s === 'COMPLETED' || s === 'DONE' || s === 'SUCCESS')
                approvedDocs += c;
            else if (s === 'REJEITADO' || s === 'REJECTED' || s === 'FAILED')
                rejectedDocs += c;
            else if (s === 'ERRO' || s === 'ERROR')
                errorDocs += c;
            else
                pendingDocs += c;
        });
...
app.get('/api/dashboard/spending-trend', requireAuth, async (req, res) => {
    try {
        const { companyId, period } = req.query;
        const userId = req.user.id;
        const userRole = (req.user.role || '').toUpperCase();
...
app.get('/api/dashboard/ai-insights', requireAuth, async (req, res) => {
    try {
        const { companyId } = req.query;
        const userId = req.user.id;
        const userRole = (req.user.role || '').toUpperCase();
        ...
        if (userRole !== 'ADMIN') {
            if (targetCompanyId) {
                if (targetCompanyId !== userAssignedCompanyId) {
                    const ownerCheck = await executeQuery('SELECT 1 FROM companies WHERE id = $1 AND owner_id = $2', [targetCompanyId, userId]);
                    if (ownerCheck.rowCount === 0)
                        return res.json([]); // Just return empty if not allowed
                }
            }
            else {
                targetCompanyId = userAssignedCompanyId;
...
app.get('/api/fiscal/saft', requireAuth, async (req, res) => {
    try {
        ...
        const xml = await saftService.generateSAFT(companyId, parseInt(month), parseInt(year));
        res.header('Content-Type', 'application/xml');
        res.header('Content-Disposition', `attachment; filename="SAFT_AO_${year}_${month}.xml"`);
        res.send(xml);
    }
    catch (err) {
        console.error('SAFT Generation Error:', err);
...
app.get('/api/reports/distribution', requireAuth, async (req, res) => {
    try {
...
app.get('/api/reports/tax-position', requireAuth, async (req, res) => {
    try {
        const { companyId, month, year } = req.query;
        let targetId = null;
        if (companyId && companyId !== 'ALL' && companyId !== 'undefined') {
...
app.get('/api/reports/vat-map', requireAuth, async (req, res) => {
    try {
        const { companyId, month, year } = req.query;
        if (!companyId)
            return res.status(400).json({ error: 'companyId is required' });
...
app.post('/api/auth/heartbeat', requireAuth, async (req, res) => {
    try {
        ...
        await executeQuery('UPDATE users SET last_seen_at = CURRENT_TIMESTAMP WHERE id = $1', [userId]);
        // 2. Atualizar ou Criar Sessão Ativa
        const activeSession = await executeQuery('SELECT id FROM user_sessions WHERE user_id = $1 AND is_active = true ORDER BY last_active_at DESC LIMIT 1', [userId]);
        if (activeSession.rowCount && activeSession.rowCount > 0) {
            await executeQuery('UPDATE user_sessions SET last_active_at = CURRENT_TIMESTAMP WHERE id = $1', [activeSession.rows[0].id]);
        }
        else {
            await executeQuery('INSERT INTO user_sessions (user_id, company_id, user_agent, is_active) VALUES ($1, $2, $3, true)', [userId, dbCompanyId, userAgent]);
        }
...
// [REMOVED] Legacy Server Plans Management - Use /api/admin/server-plans router instead
...
app.get('/api/admin/system/status', authenticate, async (req, res) => {
    try {
        res.json({
            status: 'online',
            timestamp: new Date().toISOString(),
...
// [SRE] Health Check for Container Orchestrators
app.get('/health', async (req, res) => {
    try {
        // Deep verification: DB Connection
        // Set short timeout to verify responsiveness
        const result = await executeQuery('SELECT 1 as health');
        res.json({
            status: result.rowCount ? 'ok' : 'error',
            timestamp: new Date().toISOString(),
            uptime: process.uptime()
        });
    }
...
// [SRE] Cleanup temp files in background
app.post('/api/workflow/trigger', authenticate, upload.array('files'), async (req, res) => {
    try {
        const fileCount = req.files?.length || 0;
        console.log(`[/api/workflow/trigger] Received ${fileCount} files`);
        ...
        setImmediate(async () => {
            ...
            try {
                ...
                // workflowService reads from file.path (refactored)
                await workflowService.triggerAnalysis(files, req.body.metadata, req.user);
                ...
                // [SRE] Always cleanup
                if (files) {
                    files.forEach(f => fs.unlink(f.path, (e) => { if (e)
