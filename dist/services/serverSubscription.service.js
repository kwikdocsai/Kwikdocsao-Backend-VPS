...
export class ServerSubscriptionService {
    async getSubscription(companyId) {
        const res = await executeQuery(`SELECT * FROM server_subscriptions 
             WHERE company_id = $1 AND status IN ('ACTIVE', 'SUSPENDED') 
             ORDER BY created_at DESC LIMIT 1`, [companyId]);
        return res.rows[0] || null;
    }
    async getPlans() {
        const result = await executeQuery(`SELECT * FROM server_performance_plans 
    ...
    async getCurrentAnalysisCost(companyId) {
        if (!companyId)
            return 1; // Fallback
        try {
            const res = await executeQuery(`SELECT p.analysis_cost 
                 ...
                 ORDER BY s.created_at DESC LIMIT 1`, [companyId]);
    ...
    async consumeAnalysisCredits(companyId, userId, docId, client) {
        if (!companyId)
            return 0;
        ...
        const costRes = await query(`SELECT p.analysis_cost FROM server_subscriptions s
             JOIN server_performance_plans p ON s.server_type = p.slug
             WHERE s.company_id = $1 AND s.status = 'ACTIVE' 
             ORDER BY s.created_at DESC LIMIT 1`, [companyId]);
        ...
        if (userRole === 'COLLABORATOR') {
            ...
            const updateUserRes = await query('UPDATE users SET credits = credits - $1 WHERE id = $2 AND credits >= $1 RETURNING name', [cost, userId]);
            if (updateUserRes.rowCount && updateUserRes.rowCount > 0) {
                ...
                await query(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details, company_id) 
                     VALUES ($1, $2, $3, $4, $5, $6)`, [userId, 'consume_credit', 'document_analysis', docId, JSON.stringify({ amount: cost, type: 'personal_wallet', role: 'COLLABORATOR' }), companyId]);
            }
            else {
            ...
            const updateCompanyRes = await query('UPDATE companies SET credits = credits - $1 WHERE id = $2 AND credits >= $1 RETURNING name', [cost, companyId]);
            if (updateCompanyRes.rowCount && updateCompanyRes.rowCount > 0) {
                ...
                await query(`INSERT INTO audit_logs (user_id, action, resource_type, target_id, details, company_id) 
                     VALUES ($1, $2, $3, $4, $5, $6)`, [userId, 'consume_credit', 'document_analysis', docId, JSON.stringify({ amount: cost, type: 'company_balance', role: userRole }), companyId]);
            }
            else {
    ...
    async activateSubscription(companyId, serverType, userId) {
        ...
        const result = await executeTransaction(async (client) => {
            ...
            const prevSubRes = await client.query(`SELECT s.workflow_id, s.n8n_url, p.n8n_api_key 
                 FROM server_subscriptions s
                 JOIN server_performance_plans p ON s.server_type = p.slug
            ...
            await client.query(`UPDATE server_subscriptions SET status = 'CANCELLED', updated_at = NOW() 
                 WHERE company_id = $1 AND status IN ('ACTIVE', 'SUSPENDED')`, [companyId]);
            // 3. Verify Balance
            if (currentCredits < cost) {
                throw new Error(`Créditos insuficientes. Necessário: ${cost}, Disponível: ${currentCredits}`);
            }
            // 4. Deduct Credits (Subscription Cost)
            await client.query(`UPDATE companies SET credits = credits - $1 WHERE id = $2`, [cost, companyId]);
            // 5. Award Welcome Credits (If any)
            if (plan.welcome_credits > 0) {
                await client.query(`UPDATE companies SET credits = credits + $1 WHERE id = $2`, [plan.welcome_credits, companyId]);
            }
            // 6. Create Subscription (30 Days)
            const expiresAt = new Date();
            expiresAt.setDate(expiresAt.getDate() + 30);
            const insertRes = await client.query(`INSERT INTO server_subscriptions (company_id, server_type, monthly_cost, status, expires_at)
                 VALUES ($1, $2, $3, 'ACTIVE', $4)
                 RETURNING *`, [companyId, serverType, cost, expiresAt]);
            // 7. Log Transaction: Subscription DEBIT
            await client.query(`INSERT INTO transactions (user_id, company_id, type, amount, credits, description, status)
                 VALUES ($1, $2, 'DEBIT', 0, $3, $4, 'COMPLETED')`, [userId, companyId, -cost, `Assinatura Servidor ${serverType}`]);
            // 8. Log Transaction: Welcome Credits BONUS
            if (plan.welcome_credits > 0) {
                await client.query(`INSERT INTO transactions (user_id, company_id, type, amount, credits, description, status)
                     VALUES ($1, $2, 'BONUS', 0, $3, $4, 'COMPLETED')`, [userId, companyId, plan.welcome_credits, `Bónus Inicial: Servidor ${serverType}`]);
            }
        ...
        try {
            ...
            await executeQuery(`UPDATE server_subscriptions 
                 SET n8n_url = $1, workflow_id = $2, webhook_url = $3 
                 WHERE id = $4`, [n8nDetails.n8nUrl, n8nDetails.workflowId, n8nDetails.webhookUrl, newSubscription.id]);
    ...
    async getCompanyWebhook(companyId) {
        if (!companyId)
            return "https://n8n.conversio.ao/webhook-test/document-analysis";
        try {
            const subRes = await executeQuery(`SELECT webhook_url FROM server_subscriptions 
    ...
    async processRenewals() {
        ...
        const expiringRes = await executeQuery(`SELECT * FROM server_subscriptions 
            ...
            try {
                await executeTransaction(async (client) => {
                    ...
                    if (credits >= sub.monthly_cost) {
                        // RENEW
                        await client.query(`UPDATE companies SET credits = credits - $1 WHERE id = $2`, [sub.monthly_cost, sub.company_id]);
                        // Update Expiry +30 days
                        const newExpiry = new Date(sub.expires_at);
                        newExpiry.setDate(newExpiry.getDate() + 30);
                        await client.query(`UPDATE server_subscriptions SET expires_at = $1, updated_at = NOW() WHERE id = $2`, [newExpiry, sub.id]);
                        await client.query(`INSERT INTO transactions (company_id, type, credits, description, status)
                             VALUES ($1, 'DEBIT', $2, $3, 'COMPLETED')`, [sub.company_id, -sub.monthly_cost, `Renovação Automática Servidor ${sub.server_type}`]);
                        results.renewed++;
                    }
                    else {
                        // SUSPEND
                        await client.query(`UPDATE server_subscriptions SET status = 'SUSPENDED', updated_at = NOW() WHERE id = $1`, [sub.id]);
                        results.suspended++;
                    }
                });
            }
