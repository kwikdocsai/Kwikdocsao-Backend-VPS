...
export class WatchdogAgent {
    ...
    async run() {
        ...
        try {
            ...
            const getCompanyIssue = (id) => {
                if (!companyIssues[id])
                    companyIssues[id] = { missing_fields: [], duplicates: [] };
                return companyIssues[id];
            };
            // 2. Local Validation (Missing Fields)
            // We also prepare a structure for Duplicate Checking
            const docSignatures = {}; // "nif|amount|date" -> [docId1, docId2...]
            for (const doc of res.rows) {
                ...
                if (!issuerNif)
                    missing.push('NIF Emitente');
                if (!docDate)
                    missing.push('Data Emissão');
                if (missing.length > 0) {
            ...
            for (const key in docSignatures) {
                ...
                if (ids.length > 1) {
                    ...
                    for (const id of ids) {
                        const doc = res.rows.find((r) => r.id === id);
                        if (doc) {
