...
export class AuthService {
    async register(name, email, password, role = 'ACCOUNTANT', companyId = null, ownerId = null, mustChangePassword = false) {
        ...
        const result = await executeQuery(`INSERT INTO users (name, email, password, role, company_id, owner_id, permissions, status, force_password_change, credits) 
             VALUES ($1, $2, $3, $4, $5, $6, $7, 'ACTIVE', $8, $9) 
             RETURNING id, name, email, role, company_id, owner_id, permissions, status, created_at, force_password_change, credits`, [name, email, hashedPassword, role, companyId, ownerId, JSON.stringify({
    ...
    async getTeam(adminId, companyIdFilter) {
    ...
    async updatePermissions(subordinateId, permissions, usageLimits, role) {
        const query = role
    ...
    async getTeamStats(adminId, companyIdFilter) {
        // Fetch company_id and role for the requesting user
        const adminRes = await executeQuery('SELECT company_id, role FROM users WHERE id = $1', [adminId]);
        const admin = adminRes.rows[0];
        const userRole = (admin?.role || '').toUpperCase();
    ...
    async deleteUser(userId, adminId) {
        ...
        const checkRes = await executeQuery(`SELECT u1.company_id as target_company, u2.company_id as admin_company
             FROM users u1, users u2
             WHERE u1.id = $1 AND u2.id = $2`, [userId, adminId]);
    ...
    async getMemberUsage(userId) {
        // Table audit_logs deleted
        return [];
    }
    async consumeCredit(userId, amount = 1, tool = 'unknown', client = null) {
        ...
        const walletRes = await query(client
            ? 'SELECT credits, plan, owner_id FROM users WHERE id = $1 FOR UPDATE'
            : 'SELECT credits, plan, owner_id FROM users WHERE id = $1', [walletOwnerId]);
        ...
        await query(`INSERT INTO audit_logs (user_id, action, resource_type, details) 
             VALUES ($1, $2, $3, $4)`, [userId, 'consume_credit', tool, JSON.stringify({ amount, walletOwnerId })]);
        return { credits: wallet.credits - amount };
    }
    async transferCreditsToCompany(userId, companyId, amount) {
        if (amount <= 0)
            throw new Error('O valor da transferência deve ser superior a zero');
        ...
        try {
            ...
            await client.query(`INSERT INTO transactions (company_id, user_id, credits, type, description, amount, method, status)
                 VALUES ($1, $2, $3, 'PURCHASE', $4, 0, 'TRANSFER', 'COMPLETED')`, [companyId, userId, amount, `Transferência de Créditos Pessoais para ${companyName}`]);
            // Log as transaction for USER (Debit from personal wallet)
            await client.query(`INSERT INTO transactions (company_id, user_id, credits, type, description, amount, method, status)
                 VALUES (NULL, $1, $2, 'TRANSFER_OUT', $3, 0, 'TRANSFER', 'COMPLETED')`, [userId, amount, `Transferência para Empresa: ${companyName}`]);
            // Audit for user
            await client.query(`INSERT INTO audit_logs (user_id, action, resource_type, details) 
                 VALUES ($1, $2, $3, $4)`, [userId, 'transfer_credits', 'company', JSON.stringify({
                    amount,
                    companyId,
                    companyName,
    ...
    // ==================== TEAM INVITES ====================
    async createInvite(ownerId, email, name) {
        ...
        if (!companyId)
            throw new Error('Administrador sem empresa associada.');
        ...
        const result = await executeQuery('INSERT INTO team_invites (owner_id, member_email, member_name) VALUES ($1, $2, $3) RETURNING id', [ownerId, email, name]);
        return result.rows[0].id;
    }
    async getInvites(ownerId) {
        const result = await executeQuery('SELECT * FROM team_invites WHERE owner_id = $1 ORDER BY created_at DESC', [ownerId]);
        return result.rows;
    }
    // ==================== FINANCIAL OPERATIONS ====================
    async topupCredits(userId, amount) {
        await executeQuery('UPDATE users SET credits = credits + $1, updated_at = now() WHERE id = $2', [amount, userId]);
        // Table credit_transactions deleted
        return { success: true };
    }
    async upgradePlan(userId, newPlan, creditsBonus) {
        const userRes = await executeQuery('SELECT plan FROM users WHERE id = $1', [userId]);
        const oldPlan = userRes.rows[0]?.plan;
        await executeQuery('UPDATE users SET plan = $1, credits = credits + $2, updated_at = now() WHERE id = $3', [newPlan, creditsBonus, userId]);
        await executeQuery('INSERT INTO credit_transactions (user_id, amount, type, plan_before, plan_after) VALUES ($1, $2, $3, $4, $5)', [userId, creditsBonus, 'upgrade', oldPlan, newPlan]);
        // Explicit log in transactions table for history visibility
        await executeQuery(`INSERT INTO transactions (user_id, company_id, type, amount, credits, status, description)
             VALUES ($1, NULL, 'PLAN_UPGRADE', 0, $2, 'COMPLETED', $3)`, [userId, creditsBonus, `Upgrade de Plano: ${oldPlan} -> ${newPlan} (+${creditsBonus} créditos)`]);
        return { success: true };
    }
    async setUserStatus(id, is_active) {
        return executeQuery('UPDATE users SET is_active = $1, updated_at = now() WHERE id = $2', [is_active, id]);
    }
    async changePassword(userId, newPassword) {
        const passwordHash = await hashPassword(newPassword);
        await executeQuery('UPDATE users SET password = $1, must_change_password = false, force_password_change = false, updated_at = now() WHERE id = $2', [passwordHash, userId]);
        return { success: true };
    }
    // ==================== FATURIX MODULE ====================
    async saveFaturixAudit(userId, audit) {
        const result = await executeQuery(`INSERT INTO faturix_audits (
    ...
    async getFaturixAudits(userId) {
        // Table deleted
        return [];
    }
    async getFaturixRules(userId) {
        // Retorna regras globais (sem user_id) ou específicas do usuário
        const result = await executeQuery('SELECT * FROM faturix_rules WHERE user_id IS NULL OR user_id = $1 ORDER BY created_at DESC', [userId]);
        return result.rows;
    }
    async saveFaturixRule(userId, rule) {
        const result = await executeQuery('INSERT INTO faturix_rules (user_id, name, category, description, rigor, is_active) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [userId, rule.name, rule.category, rule.description, rule.rigor, rule.isActive]);
        return result.rows[0];
    }
    async toggleFaturixRule(ruleId, isActive) {
        await executeQuery('UPDATE faturix_rules SET is_active = $1 WHERE id = $2', [isActive, ruleId]);
    }
    async getFaturixAgentSettings(userId) {
        const result = await executeQuery('SELECT * FROM faturix_agent_settings WHERE user_id = $1', [userId]);
        return result.rows[0] || { agent_instructions: '', visual_instructions: '', decision_criteria: '' };
    }
    async saveFaturixAgentSettings(userId, instructions, visualInstructions, criteria) {
        const existing = await this.getFaturixAgentSettings(userId);
        if (existing.id) {
            await executeQuery('UPDATE faturix_agent_settings SET agent_instructions = $1, visual_instructions = $2, decision_criteria = $3, updated_at = now() WHERE user_id = $4', [instructions, visualInstructions, criteria, userId]);
        }
        else {
            await executeQuery('INSERT INTO faturix_agent_settings (user_id, agent_instructions, visual_instructions, decision_criteria) VALUES ($1, $2, $3, $4)', [userId, instructions, visualInstructions, criteria]);
        }
        return { success: true };
    }
    async getFaturixStats(userId, companyId, month, year) {
        ...
        const financialsRes = await executeQuery(`SELECT 
             ...
             FROM documents
             WHERE ${whereDocs}`, docParams);
        // 5. Risks (STUBBED - Table Deleted)
        const top_risks = [];
        // 6. Chart Data (FROM documents - ACTIVE)
        const chartRes = await executeQuery(`SELECT TO_CHAR(created_at, 'YYYY-MM-DD') as date, COUNT(*) as count
             FROM documents
             WHERE ${whereDocs} AND created_at > now() - interval '7 days'
             GROUP BY date
             ORDER BY date ASC`, docParams);
        // 7. Categories (FROM documents - ACTIVE)
        const typeRes = await executeQuery(`SELECT type as doc_type, COUNT(*) as count FROM documents WHERE ${whereDocs} GROUP BY type`, docParams);
        let categories = {
            'Tax & Compliance': 0,
            'Legal Archives': 0,
            'Supply Chain': 0
    ...
    async updateFaturixAuditStatus(auditId, status, source = 'manual') {
        // Table deleted
    }
    async initializeFaturixAudit(userId, fileName, docType, companyId) {
        // Table deleted
        return { id: 'STUB' };
    }
    async completeFaturixAudit(auditId, auditData) {
        // Table deleted
        console.log(`[AUDIT_SKIP] Table faturix_audits deleted. Skipping completion for ${auditId}`);
    }
    async getFiscalEntities(userId, companyId) {
        const params = [];
    ...
    async generateSaft(companyId, month, year) {
        ...
        const docsRes = await executeQuery(`SELECT * FROM documents 
             WHERE company_id = $1 
             AND TO_CHAR(created_at, 'YYYY-MM') = $2
             AND (status LIKE 'APROVADO%' OR status = 'COMPLETED')`, [companyId, period]);
...
<AuditFile xmlns="urn:OECD:StandardAuditFile-Tax:AO_1.01_01">
    ...
    <SourceDocuments>
        <SalesInvoices>
            ...
            <TotalCredit>${docs.reduce((acc, d) => acc + parseFloat(d.valor_documento || 0), 0).toFixed(2)}</TotalCredit>
        </SalesInvoices>
        <!-- Movement of Goods -->
        <MovementOfGoods>
    ...
    async listUsers() {
        const result = await executeQuery(`SELECT id, name, email, role, status, created_at, last_login, company_id 
             FROM users 
             ORDER BY created_at DESC`, []);
        return result.rows;
    }
    async deactivateUser(id) {
        await executeQuery('UPDATE users SET status = $1, updated_at = now() WHERE id = $2', ['DEACTIVATED', id]);
        return { success: true };
    }
}
