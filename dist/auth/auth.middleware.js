...
export function checkPermissions(userPermissions, tool) {
    if (!userPermissions)
 ...
 */
export async function validateCompanyAccess(user, targetCompanyId) {
    if (!user)
        return false;
    ...
    if (String(user.companyId) === String(targetCompanyId))
        return true;
    ...
    try {
        ...
        const checkRes = await executeQuery('SELECT id FROM companies WHERE id = $1 AND owner_id = $2', [targetCompanyId, user.id]);
