...
export class BillingService {
    ...
    async createPlan(data: any) {
        ...
        const result = await executeQuery(
            `INSERT INTO plans (name, price_credits, included_credits, user_limit, analysis_cost, color, features)
             VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
            [name.toUpperCase(), price_credits, included_credits, user_limit, analysis_cost, color, JSON.stringify(features || [])]
        );
    ...
    async updatePlan(id: string, data: any) {
        ...
        const result = await executeQuery(
            ...
            [name.toUpperCase(), price_credits, included_credits, user_limit, analysis_cost, color, JSON.stringify(features || []), is_active, id]
        );
        return result.rows[0];
    }
    ...
    async createCreditPackage(data: any) {
        const { name, credits, price, bonus_credits, color, description } = data;
        const result = await executeQuery(
            `INSERT INTO credit_packages (name, credits, price, bonus_credits, color, description) 
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [name, credits, price, bonus_credits || 0, color || 'bg-blue-500', description]
        );
        return result.rows[0];
    }

    async updateCreditPackage(id: string, data: any) {
        ...
        const result = await executeQuery(
            ...
            [name, credits, price, bonus_credits, color, description, is_active, id]
        );
        return result.rows[0];
    }
    ...
    async setOperationalCost(data: any) {
        ...
        await executeQuery(
            `UPDATE operational_costs 
             ...
             WHERE resource_type = $1 AND effective_until IS NULL`,
            [resource_type]
        );

        // Inserir novo custo
        const result = await executeQuery(
            `INSERT INTO operational_costs (resource_type, cost_per_unit, unit_name, notes, effective_from) 
             VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP) RETURNING *`,
            [resource_type, cost_per_unit, unit_name, notes]
        );
        return result.rows[0];
    }
    ...
    async approvePayment(transactionId: string, adminId: string) {
        ...
        if (txRes.rowCount === 0) throw new Error('Transação não encontrada');
        const tx = txRes.rows[0];

        if (tx.status !== 'PENDING_APPROVAL') throw new Error('Transação não está pendente');
        ...
        await executeQuery(
            `UPDATE transactions 
             SET status = 'COMPLETED', 
                 ...
                 admin_notes = $2
             WHERE id = $3`,
            [adminId, 'Aprovado via Backoffice', transactionId]
        );

        return { success: true };
    }

    async rejectPayment(transactionId: string, reason: string, adminId: string) {
        try {
            await executeQuery(
                `UPDATE transactions 
                 SET status = 'REJECTED', 
                     ...
                     notes = $2
                 WHERE id = $3`,
                [`Rejeitado por ADM:${adminId}`, reason, transactionId]
            );
            return { success: true };
        } catch (error: any) {
