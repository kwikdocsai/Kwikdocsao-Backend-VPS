...
app.post('/api/faturix/analyze', requireAuth, upload.array('files'), async (req: any, res: any) => {
    try {
        ...
        try {
            ...
            // Cleanup Logic
            files.forEach(f => fs.unlink(f.path, (err) => { if (err) console.warn('Failed to delete temp file', f.path) }));
        } catch (creditErr: any) {
...
app.post('/api/documents/analyze-proxy', requireAuth, upload.array('files'), async (req: any, res: any) => {
    try {
        ...
        fetch(webhookUrl, {
            ...
            files.forEach(f => {
                fs.unlink(f.path, (err) => { if (err) console.warn('Failed to cleanup temp', f.path) });
            });
        });
...
app.get('/api/fiscal/saft', requireAuth, async (req: any, res: any) => {
    try {
        const { companyId, month, year } = req.query;
        if (!companyId || !month || !year) {
        ...
        const xml = await saftService.generateSAFT(companyId as string, parseInt(month as string), parseInt(year as string));
...
app.post('/api/workflow/trigger', authenticate, upload.array('files'), async (req: any, res: any) => {
    try {
        const fileCount = (req.files as Express.Multer.File[])?.length || 0;
        ...
        setImmediate(async () => {
            ...
            try {
                ...
                // [SRE] Always cleanup
                if (files) {
                    files.forEach(f => fs.unlink(f.path, (e) => { if (e) console.warn('Cleanup failed', f.path) }));
                }
            }
        });
        ...
        if (req.files) (req.files as Express.Multer.File[]).forEach(f => fs.unlink(f.path, () => { }));
