...
app.post('/api/webhooks/n8n/completed', async (req: any, res) => {
    try {
        const body = req.body;
        // Resilient ID extraction (Handles strings, arrays, or nested objects from N8N)
        ...
        const companyId = getVal('companyId') || getVal('company_id');

        console.log(`[WEBHOOK] Received. ID: ${docId}, User: ${userId}, Company: ${companyId}`);

        if (!docId) {
            console.warn('[WEBHOOK] No ID extracted from body:', JSON.stringify(body).substring(0, 200));
            return res.status(400).json({ error: 'No document ID found' });
        }
        ...
        if (docCheck.rowCount && docCheck.rowCount > 0) {
            ...
            return res.json({ success: true, doc: result });
        }
