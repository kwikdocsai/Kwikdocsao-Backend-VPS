    ...
    try {
        ...
        for (let body of items) {
            ...
            if (!docId && possibleFilename) {
                ...
                try {
                    // Find the most recent PROCESSING document with this name
                    const lookupRes = await executeQuery(
                        `SELECT id, uploaded_by FROM documents 
                         WHERE file_name = $1 AND status = 'PROCESSING' 
                    ...
                    if (lookupRes.rowCount && lookupRes.rowCount > 0) {
                        ...
                        console.log(`[WEBHOOK] Found docId ${docId} for file "${possibleFilename}"`);
                    } else {
                        console.warn(`[WEBHOOK] No PROCESSING document found for file "${possibleFilename}"`);
                    }
                } catch (dbErr: any) {
