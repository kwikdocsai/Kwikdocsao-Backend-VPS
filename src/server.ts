...
app.get('/api/diagnostic/debug-doc/:filename', async (req, res) => {
    try {
        ...
        const result = await executeQuery(
            'SELECT id, file_name, status, created_at FROM documents WHERE file_name ILIKE $1 ORDER BY created_at DESC LIMIT 10',
            [`%${filename}%`]
        );
...
app.post('/api/webhooks/n8n/completed', upload.any(), async (req: any, res) => {
    try {
        // Support both JSON (req.body) and Multipart (req.body + req.files)
        const payload = req.body;
        const items = Array.isArray(payload) ? payload : [payload];

        console.log(`[WEBHOOK] Received payload with ${items.length} items. Payload keys: ${Object.keys(payload || {}).join(', ')}. Files: ${req.files?.length || 0}`);
        ...
        for (let body of items) {
            ...
            const getVal = (key: string) => {
                ...
                if (body && body.json && body.json[key]) return body.json[key];
                if (body && body.output && body.output[key]) return body.output[key];
                if (body && body.metadata) {
                    try {
                        const meta = typeof body.metadata === 'string' ? JSON.parse(body.metadata) : body.metadata;
                        if (Array.isArray(meta)) {
                            const match = meta.find((m: any) => m.audit_id || m.docId || m.doc_id);
                            if (match && match[key]) return match[key];
                            if (meta[0] && meta[0][key]) return meta[0][key];
                        }
                ...
                // 5. Query Params (Backup)
                if (req.query && req.query[key]) return req.query[key];

                return null;
            };

            // Priority: audit_id, docId, doc_id
            let docId = getVal('audit_id') || getVal('docId') || getVal('doc_id') || getVal('id');
            let userId = getVal('userId') || getVal('user_id');

            // FALLBACK 1: Query Params
            if (!docId && req.query.docId) {
                docId = req.query.docId;
                console.log(`[WEBHOOK] Recovered docId ${docId} from Query Params.`);
            }

            // FALLBACK 2: Database Lookup by Filename (Last Resort)
            // The user might send "file_name" (root) or "file_id" (inside emitente)
            const possibleFilename = body.file_name ||
                body.file_id ||
                body.output?.emitente?.file_id ||
                body.output?.file_name;

            if (!docId && possibleFilename) {
                console.log(`[WEBHOOK] Searching DB for document with file_name: "${possibleFilename}"`);
                try {
                    // Find ANY document with this name (Case Insensitive, Any Status)
                    // We prioritize PROCESSING, then OTHERS.
                    const lookupRes = await executeQuery(
                        `SELECT id, uploaded_by, status FROM documents 
                         WHERE file_name ILIKE $1 
                         ORDER BY 
                            CASE WHEN status = 'PROCESSING' THEN 1 ELSE 2 END,
                            created_at DESC 
                         LIMIT 1`,
                        [possibleFilename] // e.g. "DecoAngola.png" matches "decoangola.PNG"
                    );

                    if (lookupRes.rowCount && lookupRes.rowCount > 0) {
                        docId = lookupRes.rows[0].id;
                        userId = userId || lookupRes.rows[0].uploaded_by;
                        const foundStatus = lookupRes.rows[0].status;
                        console.log(`[WEBHOOK] Found docId ${docId} for file "${possibleFilename}" (Status: ${foundStatus})`);
                    } else {
                        console.warn(`[WEBHOOK] NO DOCUMENT FOUND for file "${possibleFilename}" in DB.`);
                    }
                } catch (dbErr: any) {
                    console.error('[WEBHOOK] DB Lookup failed:', dbErr.message);
                }
            }

            if (!docId || docId === 'undefined') {
                console.warn('[WEBHOOK CRITICAL] SKIPPING ITEM: Missing "docId" and could not find by "file_name". Payload keys:', Object.keys(body || {}));
                console.warn('Dumper:', JSON.stringify(body).substring(0, 300));
                continue;
            }

            // HANDLE STRINGIFIED JSON (resultado_analise)
            // The user passes "resultado_analise" as a stringified JSON. We must parse it or existing logic fails.
            if (body.resultado_analise && typeof body.resultado_analise === 'string') {
                try {
                    const parsedAnalysis = JSON.parse(body.resultado_analise);
                    // Merge parsed data into body so completeAnalysis can find 'emitente', 'totais', etc.
                    body = { ...body, ...parsedAnalysis };
                    console.log('[WEBHOOK] Successfully parsed "resultado_analise" string.');
                } catch (e) {
                    console.error('[WEBHOOK] Failed to parse "resultado_analise":', e);
                }
            } else if (body.resultado_analise && typeof body.resultado_analise === 'object') {
                // If N8N sent it as a proper JSON object, we simply merge it up to the root
                // This ensures 'emitente', 'totais' etc are accessible to completeAnalysis
                body = { ...body, ...body.resultado_analise };
                console.log('[WEBHOOK] Successfully unpackaged "resultado_analise" object.');
            }

            console.log(`[WEBHOOK] Verified ID ${docId}. Updating Kwikdocs...`);
            try {
                // Pass the whole body object
                const result = await documentsService.completeAnalysis(docId, body, userId);
                processedResults.push(result);
                console.log(`[WEBHOOK] SUCCESS for docId ${docId}. Result:`, JSON.stringify(result));
            } catch (innerErr: any) {
                console.error(`[WEBHOOK] Failed to process doc ${docId}:`, innerErr.message);
                processedResults.push({ error: innerErr.message, docId });
            }
        }

        const responsePayload = { success: true, processed: processedResults.length, details: processedResults };
        console.log('[WEBHOOK] Sending Response to N8N:', JSON.stringify(responsePayload));
