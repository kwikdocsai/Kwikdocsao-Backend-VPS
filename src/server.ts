...
app.post('/api/webhooks/n8n/completed', upload.any(), async (req: any, res) => {
    try {
        ...
        for (let body of items) {
            ...
            // The user might send "file_name" instead of an ID.
            if (!docId && body.file_name) {
                console.log(`[WEBHOOK] Searching DB for document with file_name: "${body.file_name}"`);
                try {
                    ...
                    if (lookupRes.rowCount && lookupRes.rowCount > 0) {
                        ...
                        console.log(`[WEBHOOK] Found docId ${docId} for file "${body.file_name}"`);
                    } else {
                        console.warn(`[WEBHOOK] No PROCESSING document found for file "${body.file_name}"`);
                    }
                } catch (dbErr: any) {
