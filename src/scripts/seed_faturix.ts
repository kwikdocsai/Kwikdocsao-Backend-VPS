...
async function seedFaturixData() {
    ...
    try {
        ...
        for (let i = 0; i < recordCount; i++) {
            ...
            // Alerts
            const alerts = [];
            if (Math.random() > 0.8) alerts.push('NIF do Adquirente não consta na base');
            if (Math.random() > 0.9) alerts.push('Data fora do período fiscal');
            if (Math.random() > 0.95) alerts.push('Possível duplicidade');
            ...
            await executeQuery(
                `INSERT INTO faturix_audits (user_id, status, file_name, fiscal_data, created_at)
