...
export class PredictorAgent {
    ...
    private async analyzeCompany(client: any, companyObj: any) {
        ...
        const totals = history.map((h: any) => parseFloat(h.total));
        const avgMonthly = totals.reduce((a: number, b: number) => a + b, 0) / totals.length;
        ...
        let trend = 'STABLE';
        if (lastMonthTotal > avgMonthly * 1.1) trend = 'GROWTH';
        else if (lastMonthTotal < avgMonthly * 0.9) trend = 'DECLINE';
