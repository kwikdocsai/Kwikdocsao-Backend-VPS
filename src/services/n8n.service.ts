...
export class N8nService {
    ...
    async provisionWorkflow(clientName: string, config: N8nConfig): Promise<{ workflowId: string; webhookUrl: string; n8nUrl: string }> {
        ...
        try {
            ...
            const webhookNode = newWorkflow.nodes.find((n: any) => n.type === 'n8n-nodes-base.webhook');
            ...
            if (!path) throw new Error('Webhook node has no path defined.');
    ...
    async deleteWorkflow(workflowId: string, baseUrl: string, apiKey: string): Promise<void> {
