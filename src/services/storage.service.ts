...
export class StorageService {
    ...
    private lastConfigTime: number = 0;
    ...
    private async getClient() {
        ...
        try {
            ...
            if (res.rows.length > 0) {
                ...
                console.log('[StorageService] No DB configFound, using ENV values');
                this.initializeFromEnv();
            }
        } catch (e) {
            console.error('[StorageService] Error loading DB config, using default/env:', e);
            if (!this.client) this.initializeFromEnv();
        }

        const endpoint = await this.client!.config.endpoint();
        console.log(`[StorageService] Current Config: Endpoint=${endpoint.hostname}, Region=${process.env.S3_REGION}, Bucket=${this.bucket}, TenantId=${this.tenantId ? 'PRESENT' : 'MISSING'}`);

        return { client: this.client!, bucket: this.bucket, tenantId: this.tenantId };
    }

    async uploadFile(file: Buffer | any, fileName: string, mimeType: string, companyId: string, companyName: string, category: 'invoices' | 'receipts' | 'other-documents' = 'other-documents') {
        try {
            ...
            const isContabo = (await client.config.endpoint()).hostname.includes('contabo');
            const actualBucket = isContabo && tenantId && !bucket.includes(':') ? `${tenantId}:${bucket}` : bucket;

            console.log(`[StorageService] Uploading ${fileName} to ${key} (Bucket: ${actualBucket})...`);
            ...
            if (error.name === 'DeserializationError') {
                console.error('[StorageService] Deserialization Error - Possible non-XML response from endpoint.');
            }
    ...
    getCategoryByType(type: string): 'invoices' | 'receipts' | 'other-documents' {
        const t = (type || 'UNKNOWN').toUpperCase();
        if (t.includes('INVOICE') || t.includes('FATURA')) return 'invoices';
        if (t.includes('RECEIPT') || t.includes('RECIBO')) return 'receipts';
