    ...
    async completeAnalysis(docId: string, n8nData: any, userId?: string) {
        ...
        return await executeTransaction(async (client) => {
            ...
            if (d.resultado_analise) {
                if (typeof d.resultado_analise === 'string') {
                    try {
                        console.log(`[Analyze Debug] Parsing stringified 'resultado_analise'.`);
                        const parsed = JSON.parse(d.resultado_analise);
                        d = { ...d, ...parsed };
                    } catch (e) {
                        console.error('[Analyze Error] Failed to parse resultado_analise string:', e);
                    }
                } else if (typeof d.resultado_analise === 'object') {
                    console.log(`[Analyze Debug] Unpacking 'resultado_analise' object.`);
