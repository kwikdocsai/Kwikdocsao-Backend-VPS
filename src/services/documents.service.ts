    ...
    async completeAnalysis(docId: string, n8nData: any, userId?: string) {
        ...
        return await executeTransaction(async (client) => {
            ...
            const fiscalFields = {
                ...
                valor_iva: parseFloat(totais.total_iva || totais.iva || math.total_iva || 0),
                valor_documento: parseFloat(totais.total_geral || math.total_recalculado || d.totalAmount || 0),
                status_fiscal: decisao.status || d.status || 'VALIDADA',
                compliance_level: decisao.nivel_risco || d.risk_level || 'BAIXO',
                valor_retencao: parseFloat(totais.retencoes || totais.total_retencao || math.valor_retencao || 0)
            };
            ...
            if (companyId && resolvedUserId) {
                try {
                    ...
                    console.warn(`[Analyze] Credit consumption failed for company ${companyId}: ${creditError.message}`);
                    throw creditError;
                }
            }
