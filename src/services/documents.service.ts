...
export class DocumentsService {
    ...
    async update(id: string, data: { status?: string; type?: string; analysisResult?: any; rawResponse?: any; rawText?: string; fiscalFields?: any; fileUrl?: string; storagePath?: string; bucketName?: string }, client?: any) {
        ...
        if (data.fiscalFields) {
            ...
            if (f.nif_emitente) { fields.push(`nif_emitente = $${idx++}`); values.push(f.nif_emitente); }
            if (f.valor_base !== undefined) { fields.push(`valor_base = $${idx++}`); values.push(f.valor_base); }
        }
        ...
        (async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    console.error(`[Analyze] N8N Dispatch Failed: ${response.status} ${errorText}`);
                    // Only mark ERROR if the DISPATCH failed. 
                    // If it succeeded (200), we assume N8N is running and will call the webhook callback later.
                    await this.update(docId, { status: 'ERROR' });
                } else {
                    console.log(`[Analyze] Dispatch successful. Waiting for Async Webhook Callback...`);
                    // DO NOT call completeAnalysis here. 
                    // We rely on N8N calling /api/webhooks/n8n/completed when done.
                    // This prevents "premature" completion or race conditions.
                }

            } catch (error: any) {
                console.error(`[Analyze] Background Dispatch Error for doc ${docId}:`, error);

                // If it's a timeout, N8N might still be running? 
                // But usually safe to mark ERROR if we couldn't even talk to N8N.
                await this.update(docId, { status: 'ERROR' });
            }
        })();
    ...
    async completeAnalysis(docId: string, n8nData: any, userId?: string) {
        ...
        return await executeTransaction(async (client) => {
            ...
            if (d.resultado_analise) {
                console.log(`[Analyze Debug] Auto-unpacking 'resultado_analise' wrapper.`);
                d = { ...d, ...d.resultado_analise };
            }
