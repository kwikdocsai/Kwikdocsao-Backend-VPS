...
async function verifyComplianceLogging() {
    try {
        ...
        await pool.query(
            `INSERT INTO audit_logs (action, resource_type, target_id, details, company_id) 
             VALUES ($1, $2, $3, $4, $5)`,
            ['FISCAL_VALIDATION_BATCH', 'compliance', companyId, JSON.stringify(details), companyId]
        );

        // 3. Query logs for the company
        console.log('🔍 Consultando Feed de Conformidade (Últimos 10)...');
        const logsRes = await pool.query(
            `SELECT action, details->>'message' as msg, created_at 
             FROM audit_logs 
